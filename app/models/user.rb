class User < ActiveRecord::Base
  ...
  def name
    @name ||= facebook_profile.path '/info/name'
  end
  def nickname
    @nickname ||= facebook_profile.path '/info/nickname'
  end
  def email
    @email ||= facebook_profile.path '/info/email'
  end
  def image_url
    @image_url ||= facebook_profile.path '/info/image'
  end
  def description
    return nil unless self.fledged?
    @description ||= facebook_profile.path '/info/description'
  end

  def self.find_for_facebook_oauth(profile_hash)
    user_id = profile_hash['uid']
    if user = User.where(facebook_user_id: user_id).first
      user.update_attributes facebook_profile: profile_hash
      user
